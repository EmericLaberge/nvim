return {
  ...
  config = function()
    require("barbar").setup({
      ...
      -- Décalage pour l'explorateur de fichiers
      sidebar_filetypes = {
        NvimTree = { text = "  Explorateur", align = "left" },
      },

      icons = {
        -- C'est ici qu'on fait les coins arrondis "Bubble"
        separator = {}, 

        -- On garde un peu d'espace pour que ça respire
        separator_at_end = true,

        -- Active les icônes colorées des fichiers
    ...
    map("n", "<Space>bb", "<Cmd>BufferOrderByBufferNumber<CR>",
      { noremap = true, silent = true, desc = "Buffer Order By Number" })
    map("n", "<Space>bd", "<Cmd>BufferOrderByDirectory<CR>",
      { noremap = true, silent = true, desc = "Buffer Order By Directory" })
    map("n", "<Space>bl", "<Cmd>BufferOrderByLanguage<CR>",
      { noremap = true, silent = true, desc = "Buffer Order By Language" })
    map("n", "<Space>bw", "<Cmd>BufferOrderByWindowNumber<CR>",
      { noremap = true, silent = true, desc = "Buffer Order By Window" })
  end,
}
